...
def log_norm_describe_full(a, mode, sigma):
    ...
    if np.isinf(var):
        ...
        xmin, xmax = int(np.floor(np.log2(mode) - 5*sigma)), int(np.ceil(np.log2(mode) + 5*sigma))
...
def main(df, save_path=None, mode_bounds=(2**(-5), 2**11), ampl_bounds=(0, 10),
    ...
    melt_cols = ['voxel','varea', 'eccen', 'amplitude_estimate', 'stimulus_superclass',
...
path = 'E:\\ds003812-download\\results\\bins'
files = os.listdir(path)
print(files)
for file in files:
    pt = f"{path}\\{file}"
    for f in os.listdir(pt):
        if f.startswith('bin') and 'csv' in f:
            print(f)
            p = f"{path}\\{file}\\{f}"

            df = pd.read_csv(p)
            main(df,save_path=f"E:\\ds003812-download\\results\\tuning\\tuning_vbins\\tuning_vbins_{f}")
