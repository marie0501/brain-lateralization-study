...
flextable::save_as_image(w, path = paste0(result_dir,'table5',".png"))
...
#df <- data.frame(x = factor(tab$roiname,levels = tab$roiname), y = c(tab$eccen.Coef,tab$side.Coef,tab$`eccen:side.Coef`), group=rep(c('Excentricida','Lado','Excentricidad:Lado'),each=12))
...
#  ) + theme_bw()
#
#png(paste0(datadir2,'tamanno_efecto_roi.png'),family = "ArialMT", units = "cm",
